 ...
 *
 * Границы того, что делает этот класс:
 *
 ...
 *  - переходов между витринами не инициируем сами: если город покупателя живёт на другой
 *    витрине, мы лишь кладём его в слот предложения соседа, а кнопку нажимает человек (G4);
...
class shopPrefillPluginGeoSync
{
    ...
    public function run(): void
    {
        ...
        // Кука могла приехать с другого поддомена испорченной: соседние плагины переносят
        // её вместе со всеми остальными, но восстанавливают без наших атрибутов
        $this->storage->refreshCookie();
    ...
    private function applyTo(
        ...
        // Подстановка потребовала бы перехода между витринами либо не подействовала бы
        // вовсе. Сами не переводим (B2a), но можем предложить это соседу — он спросит человека
        if (! $adapter->canApply($target)) {
            $this->proposeTo($adapter, $target);
            return;
        }
    ...
    /**
     * Предложение перехода — на случай, когда подставить город нельзя.
     *
     * Достижимо только из ветки `DECISION_APPLY`: `BACKOFF` (покупатель менял город сам)
     * и `SKIP` (цели нет либо город уже совпадает) выходят раньше, и предлагать там нечего.
     */
    private function proposeTo(shopPrefillPluginGeoAdapterInterface $adapter, shopPrefillPluginGeoTarget $target): void
    {
        // Раз за сессию: их окно после отказа покупателя закрывается, и навязывать
        // его на каждой странице значило бы спорить с человеком
        if ($this->storage->hasProposed($adapter->getId())) {
            return;
        }

        if ($adapter->propose($target)) {
            $this->storage->markProposed($adapter->getId());
        }
    }
    ...
    public function forgetEverything(): void
    {
        foreach ($this->adapters as $adapter) {
            ...
            // Предложение снимаем всегда: записи в `applied` у него нет, и без этого
            // город прошлого покупателя остался бы висеть в чужом окне выбора города
            $adapter->forgetProposal();
