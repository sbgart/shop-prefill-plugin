...
class shopPrefillPluginZenLatch
{
    ...
    /**
     * Сверяет владельца защёлок с текущей личностью.
     *
     * `$reconciled` — поле экземпляра, а не запроса: оно защищает только повторное чтение
     * внутри одного вызова (`isExpanded()` + `sync()` на одном хуке). Блоки трёх групп
     * рендерятся в разных checkout-хуках, а объект плагина — а с ним и `ZenMode`/`ZenLatch` —
     * `waEvent::runPlugins()` пересоздаёт на каждое событие (issue-73, issue-93): второй и
     * третий хуки того же HTTP-запроса получают уже свежий инстанс с `$reconciled = false`.
     * Что их спасает от повторной сверки — не гард, а побочный эффект `waResponse::setCookie()`:
     * она правит `$_COOKIE` немедленно, поэтому второй хук читает уже обновлённого владельца,
     * совпадает с ним по `$previous === $owner` и выходит без `clearAll()`.
     *
    ...
    private function reconcile(): void
    {
        ...
        // secure/httponly — в отличие от куки самих защёлок (prefill_zen_{group}), эту
        // не читает и не пишет ни строка JS: значение серверное насквозь (issue-99).
     ...
     *
     * В куке лежит не сырой ключ ('user:<contact_id>'), а его хеш — но необратимости ради
     * соли, не ради секретности: подделка неопасна (защёлка управляет только разворотом
     * блока), HMAC не нужен. Соль нужна потому, что несолёный `substr(md5('user:'.$id), 0, 12)`
     * обратим переборoм: 2 млн вариантов id разбираются за 0.54 с на обычном CLI (issue-99).
     * Кука сама по себе теперь httponly (см. reconcile()), но это второй рубеж, не первый:
     * значение не должно раскрывать contact_id, даже если её всё же прочитают — резервной
     * копией браузерного профиля, логами прокси, будущей правкой, снявшей httponly по ошибке.
     *
    ...
    private function currentOwner(): ?string
    {
        ...
        return substr(md5($this->installSalt() . '|' . ($source_key ?? self::OWNER_ANONYMOUS)), 0, 12);
    }

    /**
     * Install-специфичная соль хеша владельца — постоянный идентификатор установки
     * (`wa-config/config.php: identity_hash`), тот же источник, которым `shopConfig` метит
     * инсталляцию перед Webasyst ID. Без соли хеш обратим и для 'user:1', и для 'user:2' с
     * любой другой установки Shop-Script — соль делает готовые радужные таблицы бесполезными.
     *
     * Ошибка чтения конфигурации не должна ронять дзен-режим целиком: при недоступной соли
     * хеш деградирует до прежнего, обратимого, но защёлки продолжают работать (B2a) —
     * молчаливый откат тут безопаснее падения.
     *
     * Откат безопасен именно потому, что значение стабильно в пределах установки:
     * `getIdentityHash()` читает статику бутстрапа и на отсутствующем ключе возвращает `''`
     * (не бросает). Установка либо всегда солёная, либо всегда нет — хеш между запросами не
     * «прыгает». Если бы прыгал, `reconcile()` видел бы смену владельца на каждом запросе и
     * снимал бы защёлки без конца: любая будущая правка источника соли обязана сохранить это
     * свойство.
     */
    private function installSalt(): string
    {
        try {
            return (string) wa('shop')->getConfig()->getIdentityHash();
        } catch (Throwable $e) {
            return '';
        }
    }
    ...
    /**
     * Ставит защёлку группы — только если браузер ещё не прислал её ровно с этим значением.
     *
     * `checkout_before_auth`/`checkout_render_*` перерисовывают развёрнутую группу почти на
     * каждом `/order/calculate/`, и без гарда `sync()` слал бы тот же `Set-Cookie` заново на
     * каждый рендер трёх групп — три лишних заголовка на запрос, ничего не меняющих
     * (issue-100 §1). Идентичность и сброс защёлок (`reconcile()`/`clearAll()`) через этот
     * метод не идут и гарда не имеют — там переписывать нужно безусловно.
     */
    private function remember(string $group): void
    {
        $name = self::COOKIE_PREFIX . $group;

        if ($this->request->cookie($name) === self::EXPANDED) {
            return;
        }

        $this->response->setCookie($name, self::EXPANDED, [
            'expires'  => 0,
            'path'     => '/',
            'samesite' => 'Lax',
        ]);
