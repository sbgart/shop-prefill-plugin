class PrefillFrontendController {
  ...
  addOrderFormEventListener() {
    ...
    $(document).on("wa_order_form_ready", function (e, form) {
      ...
      // Перехватываем update() для раскрытия секций с ошибками валидации
      if (form && form.update) {
        const originalUpdate = form.update.bind(form);

        form.update = function (options) {
          const promise = originalUpdate(options);

          promise.fail(function (state, errors) {
            if (state === "front_errors" && errors && errors.length) {
              // TODO: expandZenSectionsWithErrors(errors);
            }
          });
