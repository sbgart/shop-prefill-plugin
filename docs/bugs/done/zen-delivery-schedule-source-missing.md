...
**Статус:** исправлено 08.09.2026, подтверждено вживую.
...
`getShippingScheduleHtml()` больше не ходит в `getSelectedVariant()`. Первый источник — тот же
массив, из которого рисует расписание ядровый виджет: `vars.details.shipping_rate`. Он достижим
из хука `checkout_render_details` в обоих режимах рендера — при AJAX-запросе секции
`shopCheckoutStep::addRenderedHtml()` подставляет `$vars[$step_id] = $result` прямо перед вызовом
хука, при полной загрузке `/order/` хуки крутятся после всех шагов в
`shopCheckoutViewHelper::prepareFormVars()`, где `vars` — накопленный `$result` со всеми ключами
шагов. Прежний источник остался запасным — на случай плагина доставки, кладущего готовое
расписание прямо в тариф.

Попутно `renderPickupScheduleDays()` (переименован в `renderPickupSchedule()`, принимает весь
`pickup_schedule`, а не только `days`) догнал разметку ядра — раньше он молча терял два элемента,
и это всплыло бы сразу же, как только поле перестало быть пустым:

- пометка дня `additional` (`<span class="wa-additional">`, `details.html:255-257`) — та самая
  «Обед 13:00–14:00» из фикстуры выше;
- `title` с названием часового пояса у `.wa-time`, когда пояс ПВЗ отличается от пояса покупателя
  (`details.html:248-250`).

### Подтверждение

Живой чекаут, СДЭК ПВЗ (`sydsek`, `shop_plugin.id=43`, «ПВЗ Новосибирск, Вокзальная магистраль»),
временный `{$delivery_schedule}` в шаблоне группы `delivery`. Zen-сводка и развёрнутый ядровый
виджет на одной и той же странице дают **идентичный** список из 7 дней и один и тот же
`title="+07 Новосибирск, Омск, Новокузнецк, Понтианаке, Вьентьян"`.

Ветки, которые СДЭК не отдаёт (`additional`, совпадающие пояса, legacy `pickup_schedule_html`,
приоритет источников, пустое состояние), проверены синтетически прямым вызовом метода — все
зелёные.

### Что осталось

Расхождение с ядром теперь только в CSS: `.wa-day-wrapper` в Zen-карточке получает
`display: block` вместо ядрового `display: table`, расписание идёт столбиком без выравнивания
колонок. Это ровно та гипотеза, которую [zen-photos-css-scope-broken.md](zen-photos-css-scope-broken.md)
не мог проверить, пока поле было пустым, — теперь она подтверждена и правится в рамках того бага.
