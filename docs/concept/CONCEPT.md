...
- ✅ **Реализовано:** Галочка согласия для гостей на сохранение данных. Администратор может включить/выключить через настройку `guest/consent_required`. См. раздел [Согласие гостя на сохранение данных](#согласие-гостя-на-сохранение-данных-prefill_consent) ниже
...
**Автопродление cookie:** Cookie согласия продлевается при каждом заходе на сайт из хука `frontendHead` — явным вызовом `ConsentStorage::renewConsentIfGranted()`. `hasConsent()` — чистая проверка без побочных эффектов, используется во всех остальных местах (issue-100 §1).
