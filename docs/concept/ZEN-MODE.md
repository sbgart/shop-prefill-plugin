...
> [!NOTE]
> **Хук `checkout_render_details` вызывается всегда** — список шагов в `shopCheckoutConfig::getCheckoutSteps()` захардкожен и безусловен, настройки на состав шагов не влияют. Секция при этом может отрендериться **пустой** (короткое замыкание конвейера, `shipping.used = false`, нет адресных полей у выбранной доставки) — но хук отработает, и блок группы попадёт в разметку.
>
> Реальный риск другой: ядро **выбрасывает** вывод хука при `$details.disabled` или если кастомная тема потеряла `{foreach $event_hook.details}`. Раньше это оставляло скрывающий CSS без кнопки «Изменить» — секция исчезала навсегда. Закрыто в [issue-75](../codereview/done/issue-75-zen-collapse-without-toggle-button.md): CSS выводится вместе с кнопкой и физически не может попасть в разметку без неё. Фолбэк на `checkout_render_shipping` не нужен и невозможен — в момент рендера `shipping` неизвестно, отрендерится ли `details` дальше.
...
> [!IMPORTANT]
> **Главный принцип:** группа сворачивается, только если у неё нет ошибок валидации **и** в сессии есть минимум данных, ради которых сводку вообще имеет смысл показывать.

### Разделение источников

Три вопроса — три источника. Смешение источников и породило [баг со свёрнутой пустой доставкой](../bugs/done/zen-collapse-on-upstream-checkout-error.md).

| Вопрос | Источник | Почему именно он |
| --- | --- | --- |
| Сворачивать группу? | **Сессия** (`shop/checkout` через `SectionChecker`) | `$params` пуст для всех секций ниже упавшего шага и на каждой первой отрисовке (`fast_render`) — отвечать на этот вопрос он не умеет |
| Есть ли ошибки? | **`$params` текущего рендера** | ошибка живёт ровно один запрос, в сессии её нет |
| Что показать в сводке? | **`$params`**, при пустом — кэш сводки в сессии | названия и цены считают плагины доставки и оплаты внутри `process()`; в сессии лежат только идентификаторы |

### Приоритет проверок

```
1. Zen для группы включён?            → НЕТ: не сворачиваем
2. Кука группы == expanded?           → ДА:  не сворачиваем (намерение покупателя)
3. Есть ошибки валидации в группе?    → ДА:  не сворачиваем (покупатель должен видеть ошибку)
4. Минимум группы заполнен в сессии?  → НЕТ: не сворачиваем (нечего показывать в сводке)
5. Иначе                              → СВОРАЧИВАЕМ
```

### Минимум группы

В минимум входит **только то, что переживает короткое замыкание конвейера** — иначе группа будет разворачиваться на ровном месте. `details.street` в минимум не входит: `Details.getData()` сериализует форму, а после короткозамкнутого рендера форма пуста, и улица пропадает из сессии сама (замеры — в [баг-репорте](../bugs/done/zen-collapse-on-upstream-checkout-error.md)).
...
### Логика в PHP (`shopPrefillPluginZenMode::shouldCollapseGroup()`)
...
Кука пишется в `syncCollapseCookieState()` по факту решения: развернули — `expanded`, свернули — удалили. Поэтому **однажды открытая группа не схлопнется под руками** у покупателя, который её как раз заполняет: PHP уже записал намерение, и следующий пересчёт увидит `expanded`. Закрывает группу только сам покупатель кнопкой «Свернуть» — JS удаляет куку.

### Таблица сценариев

| Кука | Ошибки в группе | Минимум в сессии | Результат | Когда бывает |
| --- | --- | --- | --- | --- |
| нет | нет | **есть** | **Свёрнуто** | возврат покупателя с историей |
| нет | нет | нет | Развёрнуто | гость без истории, первый визит |
| нет | **есть** | любой | Развёрнуто | ошибка валидации в самой группе |
| нет | нет (ошибка выше) | **есть** | **Свёрнуто** + сводка из кэша | сломанный email: доставка цела, но `$params` пуст |
| **expanded** | любые | любой | Развёрнуто | покупатель нажал «Изменить»; или PHP уже разворачивал группу раньше |

> **Вывод:** ошибки и минимум проверяются независимо и из разных источников. Пустые обязательные поля попадают в ошибки валидации ядра, поэтому отдельной проверки «все ли поля заполнены» не нужно.
