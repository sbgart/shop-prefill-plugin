...
> [!IMPORTANT]
> **Один `<style>` тег на ВСЕ группы** — вставляется в первом хуке (`checkout_render_auth`).
> **Сводка и кнопка** — в последнем хуке каждой группы (auth для customer, details/shipping для delivery, payment для payment).

| Хук                        | Что вставляем                                        |
| -------------------------- | ---------------------------------------------------- |
| `checkout_render_auth`     | `<style>` для ВСЕХ групп + сводка/кнопка customer    |
...
| `checkout_render_payment`  | Сводка/кнопка payment (CSS уже вставлен выше)        |
...
│  │       . implode("\n", $styles) . '</style>';             │  │
│  │                                                          │  │
│  │  // 3. Выводим сводку/кнопку для customer в конце        │  │
│  │  echo $this->renderZenSummary('customer', $params);      │  │
│  └──────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
...
│  │  if (!hasDetails()) {                                    │  │
│  │      echo $this->renderZenSummary('delivery', $params);  │  │
│  │  }                                                       │  │
│  └──────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  // CSS уже вставлен выше — только сводка/кнопка         │  │
│  │  if ($this->shouldCollapseGroup('payment')) {            │  │
│  │      echo $this->renderZenSummary('payment', $params);   │  │
│  │  }                                                       │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
