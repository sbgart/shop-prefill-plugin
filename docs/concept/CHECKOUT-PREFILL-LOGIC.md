...
## Снапшот: зачем он есть

**Хранилище:** `shop/prefill_snapshot` (`SessionStorageProvider`, константа `SNAPSHOT_KEY`).

### Проблема, которую он решает

Ядро на каждом пересчёте **заменяет** секцию заказа в сессии целиком:

```php
// wa-apps/shop/lib/actions/frontend/order/shopFrontendOrder.actions.php
$session_checkout['order'] = $input;
wa()->getStorage()->set('shop/checkout', $session_checkout);
```

Не сливает — заменяет. А браузер присылает только то, что сейчас есть в DOM. Если шаг выше упал с ошибкой валидации, ядро замыкает конвейер: секции ниже рендерятся пустыми, форма пустеет, следующий пересчёт отправляет по ним пустоту — и данные исчезают из сессии, **хотя покупатель их не трогал**.

Снапшот — параллельная копия предзаполненных секций, которая этого не видит. Он восстанавливает потерянное на следующем же запросе.

### Два правила

1. **Снапшот работает на каждом запросе.** Маркер `shop/prefill_source` гасит только обращение к БД за источником, но не восстановление из снапшота. Иначе один сбой валидации стирал бы предзаполнение до конца сессии.
2. **Восстанавливаются только пустые секции** — по `SECTION_DATA_FIELDS` (без служебного `html`). Непустую секцию снапшот не трогает никогда.

### Чего он не делает

Снапшот **не возвращает то, что покупатель стёр осознанно**. Он не может отличить «покупатель очистил поле» от «данные потеряла механика ядра» — оба случая приходят одинаково, пустой секцией. Границу проводит не снапшот, а список владения `SECTION_OWNERSHIP_FIELDS`:

| Секция | Ключ `html` во владении | Что это значит |
| --- | --- | --- |
