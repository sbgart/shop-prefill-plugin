...
## Проверено вживую 09.09.2026

Витрина `wa-dev.loc/shop-1/*` (`checkout_storefront_id = 4bed657311d6ac81b8e58fdee33d1a92`) на этом
стенде уже сконфигурирована ровно под нужный случай: доставка и оплата отключены в настройках
чекаута этой витрины, а Zen для группы `delivery` включён (`shop_prefill_settings`, id 152/159) —
готовый фикстур, специально настраивать `shipping.used = false` не понадобилось.

Открыт `/shop-1/order/` с двумя товарами в корзине:

```js
document.getElementById('prefill-zen-styles-delivery')  // null — тега нет вовсе
document.getElementById('wa-step-shipping-section')      // display: none, html_len: 905
document.getElementById('wa-step-details-section')       // display: none, html_len: 1807
```

`<style id="prefill-zen-styles-delivery">` в DOM не появляется — ровно то, что предсказывало
решение issue-75 (CSS и кнопка — одна неделимая пара, а `checkout_render_details` при
`shipping.used = false` ядро выбрасывает целиком). Секции `shipping`/`details` в DOM есть (ядро их
рендерит), но целиком скрыты инлайновым/классовым `display:none` самого ядра — не нашим CSS
сворачивания, «сироты» без кнопки нет.

Побочно проверена и группа `payment` (тоже отключена на этой витрине, случай не из issue-75, но тот
же класс дефекта): `.prefill-zen-collapse-block` внутри `#wa-step-payment-section` **есть** и
показал бы `display: flex` — кнопка и сводка сгенерированы парой, как и для `customer`/`delivery`.
Просто сам `#wa-step-payment-section` дополнительно скрыт ядром целиком (`display: none`,
0 активных способов оплаты на этой витрине) — не орфан, а вторая, независимая причина скрытия поверх
уже корректной пары кнопка+CSS.

На экране это выглядит как ожидалось: «Покупатель» — рабочая свёрнутая карточка с кнопкой
«Изменить», сразу за ней — стоимость и «Подтвердить заказ», без единого пустого блока или обрезанной
вёрстки. Сценарий 5 закрыт, довесок про фикстуры/тестовые магазины из второй половины теста
(«ровно столько style, сколько свёрнутых групп») не требовался — он про количественную проверку на
обычном магазине с доставкой, уже покрыт прогонами 19-24.08.2026, упомянутыми в статусе наверху.
