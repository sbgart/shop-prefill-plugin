...
Реализация — план [delivery-variant-identity.md](../../plans/done/delivery-variant-identity.md), 5 этапов, все проверены в браузере: `shipping_type_id` удалён из `FillParams`, из сессии, из параметров заказа; вариант (`shipping_id.rate_id`) стал единственной идентичностью доставки везде — в предзаполнении, в гейте истории «Мои варианты», в дедупликации, в минимуме группы для дзен-режима.
